...
현재 M1 그래프의 자동 실행 상태를 설명하는 문서가 아니다. 아래는 기존 단계 1–23 CLI를 사용하는 저장 기록을 위한 운영 안내다. 현재 개발 상태는 [개발 진입점](development.md)을 따른다. 기존 호출명·버전 설명은 당시 기록이며 현재 스킬은 `$researchpilot`이다.
...
현재 작업 문서는 [프로젝트 문서 안내](README.md), 원본·이전 자료는 [보관함](../archive/README.md)에서 확인합니다.
...
[Korean development status snapshot (2026-09-08)](CODEX_DEVELOPMENT_STATUS_KO.md).
