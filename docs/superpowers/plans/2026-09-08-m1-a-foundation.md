...
- [ ] 신규 프로젝트만 지원하고 기존 프로젝트를 유지하는 적용 정책을 명시한다.
- [ ] 세 판단 역할의 진행 동의와 차단 쟁점 해소 조건을 구체적 예로 보여준다. 불일치를 다수결로 덮지 않는 기본값을 확인한다.
- [ ] 기본 추가 복귀 2회·한 응답 라운드의 의미와 사용자 조정 방식을 확인한다. 임의 예산을 승인된 값으로 기록하지 않는다.
- [ ] 첫 UI는 읽기 전용이며 승인은 현재 대화+CLI에서 수행한다는 범위를 확인한다.
- [ ] `command -v python3 python3.11 python3.12 python3.13 uv node`와 각 실제 버전을 확인한다. 이전 문서의 `/opt/homebrew/bin/python3.11`을 존재 확인 없이 사용하지 않는다.
- [ ] Python >=3.11을 확인한 인터프리터로 격리 환경을 만들고 `.[dev]`를 설치한다. 이후 명령의 `python`은 그 환경을 뜻한다.
- [ ] 아래 기준선 검사를 실행하고 counts·skip·warning을 그대로 기록한다.
...
- [ ] 전체 기본 회귀 `python -m pytest -q`도 변경 전 실행한다. 기존 실패는 새 기능 실패와 분리한다. 새 환경에서 이전 4,797 통과 수치를 그대로 기대값으로 사용하지 않는다.
- [ ] 기준선과 정책 결정을 문서로 묶어 검토한다. 제품 코드를 수정하지 않는다.
...
- [ ] UI 제작 전 Sites 스킬에 따라 작업공간·제작 경로를 확인한다. 최종 패키지 파일은 위 경로로 유지하고 외부 연구 데이터 배포는 하지 않는다.
- [ ] 아래 데이터는 예시임을 표시하고 demo.json을 만든다. 최상위 View 필드를 모두 포함하고 아래 식별자를 사용한다: `H1`, `H1-r1`, `H1-r2`, `I1`, `R1`, `D1`, `A1`. 내용은 '집계 평균 개선' 가설이 집단 혼합 지적을 받고 '집단별 차이 확인'으로 바뀌는 합성 예시다. 실제 논문·수치·에이전트 실행 ID를 꾸미지 않는다.
- [ ] `nodes`에는 M1 작업 열 개, `edges`에는 정상 경로와 가설/자료 복귀선, `decisions`에는 `D1`, `selected_hypothesis_ids:["H1"]`, `issue_ids:["I1"]`, 근거·응답 참조를 넣는다. 모든 UI 참조가 같은 JSON 안에서 해석되게 한다.
- [ ] 행위 검사를 먼저 작성한다.
...
- [ ] `node --test tests/ui/m1/view.test.mjs`로 초기 실패를 확인한다. ES module 해석을 위해 UI 디렉터리에 `package.json`의 `{"type":"module","private":true}`를 추가한다. 런타임 라이브러리 설치는 필요 없다.
- [ ] 최소 조회 로직을 작성하고 그래프·작업 상세·쟁점 상세·버전 비교를 연결한다.
...
- [ ] 모든 연구 텍스트는 `textContent`로 렌더링한다. DOM 선택 이벤트로 I1→R1→D1과 H1-r1→H1-r2를 이동하게 한다. 연구 텍스트를 `innerHTML`에 넣지 않는다.
- [ ] 브라우저에서 360px·736px 폭, 키보드 이동, 밝은/어두운 모드, 빈 데이터·긴 반론·존재하지 않는 참조를 확인한다.
- [ ] 시안을 사용자에게 보여주고 '왜 수정됐는지', '왜 다른 후보를 버렸는지'를 찾아보게 한다. 검사와 시안 결과를 기록하고 작업 커밋으로 묶는다.
...
- [ ] 격리된 합성 문헌 두 개와 가설 하나를 만든다. 자료 표지에 워크플로 검사용 합성 자료임을 명시한다.
- [ ] 조정자와 실제 세 판단 에이전트를 배정한다. 현재 슬롯이 충분하지 않으면 독립 배정을 순차 수행한다. 작성자와 검토자를 구분한다.
- [ ] 최초 입력에 타인의 의견을 넣지 않는다. 역할별 입력 목록과 실제 호스트 작업 ID를 기록한다.
- [ ] 세 최초 제출 후 같은 이전 라운드 스냅샷을 전달해 쟁점 응답을 받는다. 재개 실패 시 새 작업으로의 대체 관계와 전달 문맥을 명시한다.
- [ ] 다음 구조로 호스트 관측 사실을 기록한다. 원문은 실제 반환값을 사용한다.
...
- [ ] 사용자가 제출·응답의 출처를 열어보고 시안 대화와 실제 대화 차이를 확인할 수 있게 한다.
- [ ] 호스트 기록을 얻을 수 없다면 UI의 독립 실행 확인 범위를 줄이거나 구현 경로를 재검토한다. fixture를 사용해 이 작업을 통과시킨 것으로 보고하지 않는다.
- [ ] 네트워크/LLM 프로세스를 Python CLI에서 실행하지 않았는지 확인하고 실제 제약과 후속 기록 계약을 문서화한다.
...
- [ ] 총괄 노드·연결과 task 01 정책을 계약 문서에 고정한다. ID를 UI 제목과 분리한다.
- [ ] 먼저 금지 경로와 신규 버전의 순수 검사를 작성한다.
...
- [ ] `python -m pytest tests/codex_native/m1/test_contracts.py tests/codex_native/m1/test_roles.py -q`의 실패 이유를 확인한다.
- [ ] 노드와 복귀 허용 목록을 닫힌 카탈로그로 정의한다.
...
- [ ] `describe_graph`는 새 dict를 반환하고 상태 파일을 열지 않는다. 역할별 질문은 해당 노드의 목적·근거·대안을 다루며 조정자 비투표·자기 승인 금지를 명시한다.
- [ ] 반환값 변조가 다음 조회에 남지 않는 검사와 역할/권한 검사를 추가해 실행한다. 기존 `test_agent_roles.py`도 실행한다.
- [ ] A 확인 결과와 그래프·역할 계약을 기록해 커밋한다.
