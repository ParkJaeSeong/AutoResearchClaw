...
| ID | 작업 | 선행 | 사용자 확인물 | 상태 |
|---|---|---|---|---|
| A01 | 공통 기록·참조 계약 | 없음 | 어떤 기록으로 쟁점·검증·결정을 연결할지 JSON 예제를 확인한다. | 완료 · d82bce2 · 독립 검토 승인 |
| A02 | 저장 공통부와 신규 버전 격리 | A01 | 기존 M1 기록이 그대로 열리고 신규 기록은 다른 root에서만 생성된다. | 완료 · 599dcd3 · 독립 검토 승인 |
| A03 | M1 기록의 명시적 가져오기 | A02 | 현재 실제 검토 사례를 새 프로젝트에서 동일하게 조회한다. 기존 심사/승인 완료를 새 정책 통과로 간주하지 않는다. | 완료 · 663fd12 · 독립 검토 승인 |
