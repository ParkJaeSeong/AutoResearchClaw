...
export function renderResearchView(root,view,selection={},callbacks={}) {
  ...
  const atlasSlot=element('div',undefined,'atlas-slot');content.append(atlasSlot);
...
export function startApp(root,toolbar,status) {
  ...
  const atlasRoot=element('section');
  const atlasPanel=createAtlasPanel(atlasRoot,{getCurrentView:()=>view,onRefresh:()=>feed.refresh()});atlasPanel.setHistorical(head);
  ...
  setTheme(theme);
  function render(){const saved=memory(root);selection=renderResearchView(root,view,selection,{onSelection:value=>{selection=value;render();}});root.querySelector('.atlas-slot').append(atlasRoot);root.querySelector('.discovery-slot').append(discoveryRoot);restore(root,saved);}
  function controls(){toolbar.replaceChildren();toolbar.append(selectControl('기록 시점','head-select',[['','최신 기록 따라가기'],...[...knownHeads].reverse().map(h=>[h,`과거 고정 · ${h.slice(0,12)}`])],head??'',value=>{
      const target=value||null;const url=new URL(window.location.href);if(target)url.searchParams.set('head',target);else url.searchParams.delete('head');window.history.pushState({},'',url);head=target;discoveryPanel.setHistorical(head);atlasPanel.setHistorical(head);feed.selectHead(target);
  ...
  const feed=createLiveFeed({load:async selected=>{
    ...
    onView:next=>{
      ...
      root.replaceChildren(...buffer.childNodes);root.querySelector('.atlas-slot').append(atlasRoot);root.querySelector('.discovery-slot').append(discoveryRoot);view=next;selection=nextSelection;atlasPanel.update(next);
      knownHeads=[...new Set([...knownHeads,...next.heads.map(h=>h.head_id)])];controls();restore(document.body,saved);
    },
    onStatus:info=>{status.textContent=info.connected?`${head?'과거 버전 유지 · 저장 기능 잠김':'연결됨 · Atlas 답변과 판단 저장 가능'}${info.currentHead && view && info.currentHead!==view.head_id?' · 최신 HEAD가 별도로 있습니다':''}`:`연결 끊김 · ${info.error} · 마지막 정상 기록을 유지하며 재연결합니다.`;status.className=info.connected?'connection':'connection warning';}});
