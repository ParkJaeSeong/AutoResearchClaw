...
const labels={summary:'요약',queries:'검색에 사용한 말',next_queries:'다음 검색어',disagreements:'의견 차이',gaps:'남은 공백',sufficient:'자료를 더 찾아야 하는지',
 url:'원문 주소',doi:'DOI',query:'에이전트가 보고한 발견 검색어',query_observed:'검색어 관측 여부',access_status:'접근 상태',reading_scope:'읽은 범위',finding:'이 자료에 대한 의견',limitations:'해석의 한계',interpretation:'이 자료에 대한 의견',access_level:'읽은 범위',
...
export function createDiscoveryPanel(root) {
  ...
  return {update,setHistorical(value){historical=Boolean(value);root.hidden=historical||snapshot?.available===false;},setStatus(info){status.textContent=info.connected?'탐색 기록이 연결되어 있습니다':'탐색 연결 끊김 · 마지막 정상 탐색 결과를 유지하며 재연결합니다.';status.className=info.connected?'muted':'warning';}};
}
