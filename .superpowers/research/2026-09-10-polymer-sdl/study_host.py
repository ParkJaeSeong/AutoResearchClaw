"""Real-topic M1 scoping host; public CLI only, no corpus authority or experiments.
Adapted from the previously verified CLI orchestration, not its synthetic data.
"""
...
def host_review(label, role, packet, materials):
    ...
    persona = {
        'domain': '고분자 복합소재 전문가로서 조성·공정·분산·계면·측정 조건과 물성의 관계, 대표 사례의 타당성을 검토한다.',
        'methodology': '가설 검증·통계 전문가로서 경쟁 가설, 데이터 누출, 연구실·배치 외삽, 불확실성 교정, 공정한 비교와 인과 주장의 한계를 검토한다.',
        'critical': 'SDL 구현과 반증을 검토한다. 물성 최적화와 지식 획득을 구분하고 계량·혼합·성형·측정의 자동화 병목, 실패·추적성·장비 가정을 점검한다.',
    }[role]
    prompt = f'''당신은 실제 연구 검토자 {role}입니다. {persona}
사용자가 제시한 실제 연구 주제의 M1 범위 탐색입니다. 합성 실험값이나 장비·예산·사용자 승인을 만들어내지 마세요.
현재 목표는 대표 소재·물성과 검증할 질문을 문헌 비교로 선정할 연구 범위를 평가하는 것입니다. 아직 없는 데이터·장비는 미확정으로 보존하세요.
문헌의 초기 요약은 저자의 예비 해석이지 검증된 corpus가 아닙니다. 직접 읽지 않은 논문을 검증했다고 말하지 마세요. 주장이나 신규성을 과장하지 마세요.
...
def council(artifact):
    ...
    author = dict(id=uid(), project_id=project, actor_id='study-author', role='owner', milestone='M1', active=True)
