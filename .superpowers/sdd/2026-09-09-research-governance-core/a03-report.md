...
Confirmed that legacy `_by_assignment` silently treats unsupported disclosed-response/final collection types as empty, potentially dropping canonical resolution. Added a validation-only boundary before `build_issue_threads`: all three disclosed rounds must be lists of objects; nested issue/response/disposition collections must also be lists of objects; response/disposition issue IDs and final assignment IDs must be nonempty strings; final statuses must be canonical open/resolved. Missing required collection fields are rejected. No accepted-source projection, manifest, identity or receipt field changed, preserving valid same-command replay.

RED: expanded malformed disclosure tests produced 16 failures from unexpected acceptance and 26 passing pre-existing/guarded cases. GREEN: migration+CLI+M1 views 61 passed in 9.57s; full research_graph suite 152 passed in 1.18s. Every rejection fixture asserts no target creation and unchanged source bytes/mtime/modes. `git diff --check` clean. Actual source/acceptance targets remain untouched by worker.
