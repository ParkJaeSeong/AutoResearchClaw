...
The current milestone supports stages 1–11 through resource planning, the explicit Stage-12 research-result handoff and registration boundary, and a bounded Stage-13 refinement protocol. Codex performs the declared work; the local CLI persists state, validates formats, records hash-bound approvals, and reports evidence. It does not receive external LLM credentials or start another model process. Stage 10 authors and statically validates a computational package but does not execute it. Stage 11 plans resources without execution. Stage 12 approval records a decision but does not execute the experiment. Stage 13 is a user-mediated council refinement only; paper drafting and later orchestration are not implemented.
...
23. For exactly one explicit user request to refine a Stage-13 project, read [references/refinement.md](references/refinement.md) before acting. Follow its council roles, disclosure order, confirmation gates, envelope escalation route, and Task-7-only CLI argv. Stop after the Stage-14 handoff.
24. Never run the deferred research argv in Stage 11. Do not execute generated code or create `experiment/results.json` yourself; only the user may run the returned approved argv. Do not install packages, download data, access a network, call an LLM API, or configure an LLM provider. Explicit approval is a hash-bound decision only; it does not execute the research experiment.
