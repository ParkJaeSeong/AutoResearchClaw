...
The current milestone supports stages 1–10 through an approved computational validation package. Codex performs the research and writes the declared artifacts; the local CLI persists state, validates formats, records hash-bound approval, and reports evaluation metrics. It does not receive external LLM credentials or start another model process. Stage 10 authors and statically validates a computational package but does not execute it. Policy-evidence and laboratory Stage 10 packages, Stage 11 resource planning, validation execution, paper drafting, and later orchestration are not implemented, so do not claim or attempt them through this workflow.
...
14. After valid stage-10 output, run `resume ROOT --json` and `evaluate ROOT --json`, report only the completed computational-package milestone and its six declared outputs, and stop before unsupported Stage 11. Do not prepare Stage 11, execute the design, generate results, or draft a paper.
