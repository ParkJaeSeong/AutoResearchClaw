...
The current milestone supports stages 1–7 through evidence synthesis. Codex performs the research and writes the declared artifacts; the local CLI persists state, validates formats, records hash-bound approval, and reports evaluation metrics. It does not receive external LLM credentials or start another model process. Stage 8 hypothesis generation, experiment execution, paper drafting, and later orchestration are not implemented, so do not claim or attempt them through this workflow.
...
6. When the current packet is stage 7, read [references/synthesis.md](references/synthesis.md), use only the validated extraction corpus, and write only the declared synthesis artifact.
...
11. After valid stage-7 output, run `evaluate ROOT --json`, report only the completed synthesis milestone and its declared artifact, and stop before stage 8. Do not generate hypotheses, experiments, or a paper.
