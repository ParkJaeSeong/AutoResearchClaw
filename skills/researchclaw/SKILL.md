...
18. Before asking for Stage-12 approval, direct the user to run the package's known-answer self-test with the current verified absolute interpreter and exact declared argument array. Register the resulting report explicitly with `experiment register-self-test ROOT --report experiment/self_test_report.json --confirm-self-test --json`. The authoritative argv array, not a quoted display string, controls execution. Present the registered report and ready plan; the user must review and decide approval.
...
21. For an existing unregistered result, use only the confirmation-gated quarantine and cleanup commands reported by `resume`; do not move or delete it by hand. A published partial quarantine temp is preserved and never resumed or written; recovery uses a fresh inode if capacity permits and otherwise fails closed for manual/operator action. A complete read-only candidate may be verified and published without mutation.
