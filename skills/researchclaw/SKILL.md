...
The current milestone supports stages 1–11 through resource planning. Codex performs the research and writes the declared artifacts; the local CLI persists state, validates formats, records hash-bound approval, and reports evaluation metrics. It does not receive external LLM credentials or start another model process. Stage 10 authors and statically validates a computational package but does not execute it. Stage 11 plans resources without execution. Stage 12 is an approval-only, unsupported execution boundary: an explicit approval records a decision but does not execute the experiment. Paper drafting and later orchestration are not implemented.
...
18. Stop. Never run the deferred command in Stage 11. Do not execute generated code, create `experiment/results.json`, install packages, download data, access a network, call an LLM, or spawn an agent. Explicit approval is a hash-bound decision only; it does not execute the research experiment.
