...
The durable state defines 23 ordered contracts, and the current Codex-native CLI can prepare and validate stages 1–11. A valid stage 11 is `11 / 23`; it is not a finished research project, executed validation, or paper. Stage 12 is an approval-only unsupported execution boundary: show Stage-11 readiness and obtain an explicit user decision, but approval records a hash-bound decision and does not execute the experiment. Do not create a separate final report or write to a shared sibling output directory. Stages 12–23 are not implemented for execution in this milestone.
