...
5. Repeat the three independent assessments and one challenge/revision round over the registered candidate evidence. Register the 2–1 final selection, retain the dissenting role and rationale, and finalize only after the user confirms it. A successful finalization completes Stage 13 and advances to the dedicated Stage 14 protocol in [result-analysis.md](result-analysis.md). Any pre-finalization wording retained in a council limitation is historical context; the registered final selection and completed Stage 13 are the current-state authority.
