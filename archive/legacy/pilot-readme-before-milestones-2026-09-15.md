...
제품명은 **Pilot**입니다. 기존 설치·연구 기록과의 호환성을 위해 Python 패키지와 CLI(`researchclaw-codex`), 연구 저장 경로는 유지합니다. 프로젝트 스킬 호출명은 `$researchpilot`입니다. GitHub 주소는 실제 저장소 주소를 사용합니다.
...
the user invokes `$researchpilot` or clearly requests the Pilot (ResearchClaw) workflow by name.
...
[computational package authoring reference](skills/researchpilot/references/computational-package.md).
...
```text
$researchpilot Start a materials research project on formation-energy prediction.
```
...
Use this workflow only after an explicit `$researchpilot` or named ResearchClaw
...
[Stage-15 decision reference](skills/researchpilot/references/research-decision.md).
...
or automatic approval; their instructions do not apply to `$researchpilot`.
