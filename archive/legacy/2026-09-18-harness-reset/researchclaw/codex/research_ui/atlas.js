...
import {createAtlasService} from './atlas_service.js';
...
async function post(path,payload){
  const response=await projectFetch(path,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
...
export function createAtlasPanel(root,options={}) {
  ...
  const mutationBusy=new Set(),retryPayloads=new Map(projectId()?(readProjectState().atlasRetries??[]):[]);
  const persistRetries=()=>projectId()&&saveProjectState({atlasRetries:[...retryPayloads]});
  ...
  const header=element('header');header.append(element('p','ATLAS 연결','eyebrow'),element('h2','Atlas에 질문하고 답변 가져오기'),element('p','Atlas의 답변과 출처를 가져와 이 연구에서 어떻게 사용할지 정리합니다.','muted'));
  const intake=element('details',undefined,'atlas-intake'),fileLabel=element('label','Atlas 답변 파일 (QA Markdown)','atlas-control'),fileInput=element('input');fileInput.type='file';fileInput.accept='.md,text/markdown';fileInput.dataset.key='atlas-file';fileLabel.append(fileInput);
  const intakeStatus=element('p','파일을 선택하면 내용을 먼저 확인합니다.','muted'),mutationStatus=element('p','내용을 확인한 뒤 저장하세요.','muted'),retryRoot=element('section',undefined,'atlas-retries'),previewRoot=element('div'),importButton=button('이 연구에 가져오기',importFile,'atlas-import');importButton.disabled=true;
  intake.dataset.key='atlas-file-import';intake.append(element('summary','저장된 답변 파일로 가져오기'),fileLabel,intakeStatus,previewRoot,importButton,mutationStatus,retryRoot);
  const records=options.recordsRoot??element('section'),forms=element('section',undefined,'atlas-forms');const serviceRoot=element('section');root.append(header,serviceRoot,intake);if(!options.recordsRoot)root.append(records);root.append(forms);
  const servicePanel=createAtlasService(serviceRoot,{request,getCurrentView,onRefresh:options.onRefresh});
  ...
  async function previewFile(){
    ...
    catch(error){if(generation!==fileGeneration)return;status(intakeStatus,`미리보기를 불러오지 못했습니다. 파일은 가져오지 않았습니다. ${error.message}`,'error');}
    finally{if(generation===fileGeneration){previewBusy=false;drawPreview();gate();}}
  }
  function drawPreview(){
    previewRoot.replaceChildren();if(!preview)return;
    const imported=(view?.external_evidence??[]).some(e=>e.record?.sha256===preview.file_sha256);const box=element('article',undefined,'atlas-preview');box.append(element('h4',preview.question??'질문 없음'),badge(imported?'가져온 답변 미리보기':'가져오기 전 미리보기'));
    ...
    if(preview.missing_fields?.length)box.append(element('p',`답변에 없는 항목: ${preview.missing_fields.join(', ')}`,'pending'));
  ...
  function drawRetries(){retryRoot.replaceChildren();retryButtons.length=0;for(const [path,payload] of retryPayloads){const row=element('article',undefined,'callout');row.append(element('strong',`${operationName(path)} 결과 확인 필요`),element('p',retrySummary(path,payload),'prose'),element('p','아래 버튼은 같은 내용과 command ID로 저장 결과를 다시 확인합니다. 결과를 확인할 때까지 새 내용은 저장할 수 없습니다.','muted'));const retry=button('이 요청 그대로 다시 확인',()=>mutate(path,payload,retry,true),`atlas-retry-${path.split('/').at(-1)}`);retry.dataset.path=path;retryButtons.push(retry);row.append(retry);retryRoot.append(row);}}
  async function mutate(path,payload,buttonNode,explicitRetry=false){
    if(historical||mutationBusy.has(path)||retryPayloads.has(path)&&!explicitRetry)return;const sent=retryPayloads.get(path)??payload;mutationBusy.add(path);retryPayloads.set(path,sent);persistRetries();buttonNode.disabled=true;
    ...
    catch(error){if(error.status>=400&&error.status<500){retryPayloads.delete(path);status(mutationStatus,`저장되지 않았습니다. 입력을 고친 뒤 다시 시도하세요. ${error.message}`,'error');if(error.status===409){try{await options.onRefresh?.();}catch{}}}else{retryPayloads.set(path,sent);status(mutationStatus,`저장 결과를 확인하지 못했습니다. 아래 요청의 다시 확인 버튼을 누르세요. ${error.message}`,'warning');}}
    finally{mutationBusy.delete(path);persistRetries();drawRetries();gate();}
  }
  ...
  function drawRecords(){
    ...
    if(decisions.length){records.append(element('h3','조정자 결정'));for(const entry of decisions){const r=entry.record,card=element('article',undefined,'atlas-record');card.id=anchorId('decision',r.id);const review=reviews.find(x=>sameRef(x.ref,r.review_ref)),used=evidence.find(x=>sameRef(x.ref,review?.record?.evidence_ref)),prior=decisions.find(x=>sameRef(x.ref,r.prior_ref));card.append(element('h4',r.title??r.id),badge(r.review_status==='council_final'?'에이전트 검토 후 최종 판단':'조정자 기록 · 연결된 에이전트 의견 없음',r.review_status==='council_final'?'neutral':'pending'),jump('근거 사용 판단 보기',anchorId('review',review?.record?.id)),element('p',r.conclusion??'결론 미제공','prose'),element('p',r.rationale??'이유 미제공','prose'),renderValue(view,r.limitations??[]));if(prior)card.append(jump('이어 쓴 이전 결정 보기',anchorId('decision',prior.record.id)));if(r.submission_refs?.length){const submissions=element('details');submissions.append(element('summary','결정에 참고한 에이전트 의견'),renderValue(view,r.submission_refs));card.append(submissions);}if(used?.newer_ref)card.append(element('p','이 결정은 이전 Atlas 답변을 근거로 내렸습니다. 새 답변을 검토한 뒤 기존 결정을 유지할지 수정할지 정하세요.','callout'));records.append(card);}}
    if(view?.m1_preparations?.length){
      records.append(element('h3','실험 착수 준비'));
      const names={design_analysis:'설계·분석',data_lineage:'데이터 출처와 가공 이력',material:'재료',manufacturing:'제작·가공',measurement:'측정',repetition:'반복 실험',resources_permissions:'인력·자원·권한'};
      for(const entry of view.m1_preparations){
        const r=entry.record,card=element('article',undefined,'atlas-record');card.id=anchorId('preparation',r.id);
        const ready=entry.current&&!entry.superseded&&entry.preparation_ready;
        card.append(element('h4',entry.superseded?'이전 준비 기록':'실험 준비 확인'),badge(ready?'준비 증거 확인됨':'준비 확인 필요',ready?'ready':'pending'));
        if(!entry.current)card.append(element('p','검토에 사용한 자료가 바뀌었습니다. 다시 검토해야 합니다.','callout'));
        card.append(element('p',ready?'항목별 준비 근거를 확인했습니다. M2로 넘겼는지, 받는 쪽에서 수락했는지는 인계 기록에서 확인하세요.':'준비 완료와 실험 실행 승인은 아직 확인되지 않았습니다.','prose'),element('p',ready?'다음: 최종 검토와 남은 문제·자원 조건을 확인한 뒤 M2 인계를 진행합니다.':'다음: 담당자는 미확인 항목의 근거를 찾아 설계 검토에 추가해야 합니다.','prose'));
        if(entry.review_ready===false)card.append(element('p','설계 검토에도 확인할 조건이 남아 있습니다.','pending'));
        const list=element('ul');
        for(const [name,item] of Object.entries(r.items??{})){
          const row=element('li',`${names[name]??name} · ${!entry.current?'재검토 필요':item.status==='verified'&&item.verification_refs?.length?'검증 기록 있음':item.status==='not_applicable'?(item.verification_refs?.length?'적용하지 않는 이유를 검토함':'적용하지 않는 이유만 기록함'):'미확인'} — ${item.reason??'확인 이유 미제공'}`);
          for(const ref of item.verification_refs??[]){
            const check=view.m1_preparation_verifications?.find(entry=>sameRef(entry.ref,ref));
            if(!check)continue;
            const evidence=view.m1_preparation_evidence?.find(entry=>sameRef(entry.ref,check.record.evidence_ref));
            const detail=element('details');detail.dataset.key=`atlas-preparation-check:${r.id}:${name}:${check.record.id}`;
            detail.append(element('summary',`${names[name]??name} 확인 근거`),element('p',check.record.rationale,'prose'));
            detail.append(element('blockquote',check.record.excerpt,'prose'));
            if(evidence){detail.append(element('p',`자료 버전: ${evidence.record.source_version}`));if(evidence.source_ref)detail.append(renderValue(view,evidence.source_ref));}
            detail.append(renderValue(view,ref));row.append(detail);
          }
          list.append(row);
        }
        card.append(list);
        for(const [kind,title] of [['design','설계·분석 결정 보기'],['data_lineage','데이터 이력 결정 보기']]){
          const decision=decisions.find(d=>sameRef(d.ref,r.decision_refs?.[kind]));
          if(decision){const action=element('p');action.append(jump(title,anchorId('decision',decision.record.id)));card.append(action);}
        }
        const detail=element('details');detail.dataset.key=`atlas-preparation:${r.id}`;
        detail.append(element('summary','준비 기록과 담당·근거 확인'),renderValue(view,r),element('p','자료를 확인한 담당자와 검토 결과를 연결합니다. 준비 확인만으로 실험이 시작되지는 않습니다.','muted'));
        card.append(detail);records.append(card);
      }
    }
    if(questions.length){records.append(element('h3','저장한 추가 질문 초안'));for(const entry of questions){const r=entry.record,card=element('article',undefined,'atlas-record'),text=questionText(r),decision=decisions.find(x=>sameRef(x.ref,r.decision_ref));card.append(element('h4',r.question??r.id),jump('연결된 결정 보기',anchorId('decision',decision?.record?.id)),element('pre',text,'source-text'));const copy=button('이 질문 전체 복사',async()=>{try{if(!globalThis.navigator?.clipboard?.writeText)throw Error();await globalThis.navigator.clipboard.writeText(text);status(intakeStatus,'질문 전체를 복사했습니다. Atlas로 보내려면 질문 입력란에 붙여 넣으세요.');}catch{status(intakeStatus,'자동 복사를 사용할 수 없습니다. 표시된 전체 내용을 선택해 복사하세요.','warning');}},`atlas-copy:${r.id}`);card.append(copy);records.append(card);}}
  }
  function gate(){
  ...
  function update(next){view=next;drawRecords();drawForms();drawPreview();drawRetries();gate();}
  function setHistorical(value){historical=Boolean(value);servicePanel.setHistorical(historical);gate();}
  return {update,setHistorical,refresh:()=>servicePanel.refresh()};
}
