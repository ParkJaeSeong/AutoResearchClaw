...
export function renderResearchGraph(root,view,selection,onSelect=()=>{}) {
  root.replaceChildren();root.append(element('h2','M1 작업 지도'),element('p','M1은 자료 검토부터 가설·실험 설계·착수 준비까지 이어집니다.','muted'),element('p','단계를 선택하면 해당 내용과 대화를 볼 수 있습니다.','muted'));
  ...
  const moves=element('ol',undefined,'move-list');for(const move of view.transitions){const item=element('li');
    item.append(button(`${move.from_node?label(move.from_node):'시작'} → ${label(move.to_node)}${move.from_node===move.to_node?' · 같은 단계 수정':''}`,()=>onSelect(move.to_node,move.revision_id),`move:${move.revision_id}`),element('small',`HEAD ${move.head_id.slice(0,12)}`));moves.append(item);}
