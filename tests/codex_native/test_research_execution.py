...
def test_register_result_completes_stage_twelve(tmp_path):
    ...
    assert status.manifest_path in reopened.state.artifacts
    assert (
        f".researchclaw/evidence/objects/{status.result_object_sha256}"
        in reopened.state.artifacts
    )
    assert (
        reopened.state.artifacts["experiment/results.json"].sha256
...
def test_register_result_is_immutable_when_result_changes_during_state_persistence(
    tmp_path, monkeypatch
):
    project = build_approved_stage_twelve_project(tmp_path / "project")
    prepare_research_execution(project)
    ...
    monkeypatch.setattr(StateStore, "save", persist_after_replacing_result)
    ...
    assert reopened.state.current_stage == 13
    assert (
        project.root / f".researchclaw/evidence/objects/{status.result_sha256}"
    ).is_file()
...
def test_register_result_repairs_owned_partial_success_event_tail(
...
def test_result_mutation_during_success_append_does_not_change_immutable_registration(
    tmp_path, monkeypatch
):
    project = build_approved_stage_twelve_project(tmp_path / "project")
    prepare_research_execution(project)
    ...
    assert (
        sum(
            event.type == "research_result_registration_rolled_back" for event in events
        )
        == 0
    )
    assert not (
...
@pytest.mark.parametrize(
    "tamper",
    ("target_topic", "target_retry_counts", "result_ref", "event_metric_count"),
)
@pytest.mark.skip(reason="superseded by immutable pending semantic tamper coverage")
def test_pending_recovery_rejects_noncanonical_state_or_event_binding(
    ...
    if tamper == "target_topic":
        pending["target_state"]["topic"] = "tampered topic"
    elif tamper == "target_retry_counts":
        pending["target_state"]["retry_counts"] = {"12": 999}
    elif tamper == "result_ref":
        pending["target_state"]["artifacts"]["experiment/results.json"]["sha256"] = (
            "0" * 64
        )
    else:
        pending["success_event"]["payload"]["metric_count"] = 999
...
@pytest.mark.skip(
    reason="legacy journal fragment shape; immutable journal tested separately"
)
...
@pytest.mark.parametrize("recovery", ("register", "handoff"))
@pytest.mark.skip(
    reason="legacy rollback-event protocol replaced by durable abort intent"
)
def test_aborting_registration_never_resurrects_after_compensation_interruption(
    ...
    def interrupt_after_rollback(self, event, *, expected_offset):
        ...
        if event.type == "research_result_registration_failed" and rollback_written:
...
def test_pending_recovery_recomputes_jointly_tampered_counts_and_event_digest(
    ...
    pending["rollback_event"]["payload"]["registration_event_sha256"] = hashlib.sha256(
        success_bytes
    ).hexdigest()
    pending_path.write_text(
        json.dumps(pending, sort_keys=True, separators=(",", ":")),
...
@pytest.mark.skip(reason="superseded by immutable pending closed-schema coverage")
...
def test_pending_recovery_rejects_oversize_before_json_decode(tmp_path, monkeypatch):
    project = build_approved_stage_twelve_project(tmp_path / "project")
    prepare_research_execution(project)
    contract = load_execution_contract(project.root)
    write_contract_bound_research_result(project, contract)
    ...
    monkeypatch.setattr(
        evidence_registration, "_decode_pending_json", json_decode_must_not_run
    )
...
def test_register_result_returns_idempotent_status_without_state_change(tmp_path):
    project = build_approved_stage_twelve_project(tmp_path / "project")
    prepare_research_execution(project)
    contract = load_execution_contract(project.root)
    write_contract_bound_research_result(project, contract)
    ...
    duplicate = register_research_result(project, "experiment/results.json")

    assert (project.root / ".researchclaw/state.json").read_bytes() == state_before
    assert duplicate.current_stage == 13
    reopened = ResearchProject.open(project.root)
    assert reopened.state.current_stage == 13
    assert reopened.state.completed_stages.count(12) == 1
    events = event_log_for(project.root).read_all()
...
def test_validate_research_result_accepts_exact_binding_without_mutation(tmp_path):
    ...
    assert (
        validated.result_sha256 == hashlib.sha256(result_path.read_bytes()).hexdigest()
    )
...
def test_research_execution_rejects_wrong_stage_twelve_approval_project_id(
    ...
    controls = {
        relative: (project.root / relative).read_bytes() for relative in control_paths
    }
...
def test_validate_research_result_rejects_duplicate_json_keys(tmp_path):
    ...
    result_path.write_bytes(
        result_path.read_bytes().replace(
            b'"status": "completed"', b'"status": "partial", "status": "completed"'
        )
    )
    state_before = (project.root / ".researchclaw/state.json").read_bytes()
    result_before = result_path.read_bytes()
...
def test_prepare_run_writes_bound_contract_without_executing_project_code(tmp_path):
    ...
    assert status.result_path == "experiment/results.json"
    assert (
        status.contract_sha256 == hashlib.sha256(contract_path.read_bytes()).hexdigest()
    )
